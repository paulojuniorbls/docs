...
<AccordionGroup>
  ...
  <Accordion icon="message-bot" title="Enviando uma mensagem de texto">
    Para enviar uma mensagem de texto simples utilizando o componente **euAtendo**, você deve usar a função `EnviarTexto`. Certifique-se de que a instância está criada e conectada antes de realizar o envio.
    ...
    begin
      ...
      if not ApiEuAtendo1.EnviarTexto('+5511999999999', 'Olá! Como posso ajudar?', ErrorMsg) then
      begin
  ...
  <Accordion icon="code" title="Enviando um texto com anexo em base64">
    Se o anexo estiver em formato base64, você pode utilizar a função `EnviarAnexoBase64`. Este método é útil quando o arquivo já está codificado em base64, como em integrações mais avançadas.

    Exemplo de como enviar uma mensagem com um anexo codificado em base64:
    ...
    var
      ...
      Base64Anexo: string;
    begin
      // Exemplo de um anexo base64 (substitua pelo seu conteúdo)
      Base64Anexo := 'JVBERi0xLjcKJcfsj6IK...'; // O conteúdo completo em base64 do arquivo.

      // Enviar texto e anexo em base64
      if not ApiEuAtendo1.EnviarAnexoBase64('+5511999999999', 'Aqui está o anexo em base64:', 'documento.pdf', Base64Anexo, ErrorMsg) then
      begin
    ...
    Neste exemplo, o anexo é enviado em base64, permitindo maior flexibilidade em integrações que exigem esse formato.
  </Accordion>
