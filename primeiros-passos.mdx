...
<AccordionGroup>
  ...
  <Accordion icon="signal" title="Verificando o status da instância">
    Para verificar o status atual de uma instância (aberta, fechada, conectando), utilize a função `StatusInstancia`. O evento de retorno fornecerá as informações sobre o nome da instância e o estado dela.

    Exemplo de como verificar o status da instância:
    ...
    Em seguida, implemente o evento que será disparado após a chamada da função:
    ...
    begin
      ...
      // Exibir uma mensagem ao usuário de acordo com o estado da instância
      if InstanceStatus.State = 'open' then
        ShowMessage('A instância está conectada.')
      else if InstanceStatus.State = 'closed' then
        ShowMessage('A instância está fechada.')
      else if InstanceStatus.State = 'connecting' then
        ShowMessage('A instância está tentando conectar.');
    end;
    ```

    Neste exemplo, o estado da instância pode ser um dos seguintes valores:
    - **open**: Instância conectada.
    - **closed**: Instância desconectada.
    - **connecting**: A instância está em processo de conexão.

    O retorno deste evento permite que o usuário saiba o status atual de qualquer instância conectada ao **euAtendo**.
  </Accordion>
