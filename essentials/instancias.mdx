...
<AccordionGroup>
  ...
  <Accordion icon="mobile" title="Conectar a uma Instância já Criada">
    ...
    #  H1 Title
    # 📱 Conectar a uma Instância já Criada

		Cada instância no **euAtendo** representa um dispositivo conectado, como o celular de um cliente. Após criar uma instância, é possível reconectar-se a ela utilizando o nome e a chave da API previamente salvos.

		## Pré-requisitos

		- O nome da instância e a chave da API devem estar salvos no banco de dados ou em variáveis persistentes.
	- O componente **ApiEuAtendo** deve estar devidamente configurado para realizar a conexão.

	## Exemplo de Código

	Aqui está um exemplo de como conectar a uma instância já criada, utilizando Delphi:
